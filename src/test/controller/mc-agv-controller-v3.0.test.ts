...
import { createHeaderlessOrder, createPickDropNoopAction, testOrder, testOrderError } from "./mc-agv-controller-helpers";

initTestContext(tap);

(async () => {
    await tap.test("Master Controller VDA V3.0 - AGV Controller VDA V3.0", async t => {
        ...
        // V3.0 state transformation is now handled by AgvController._transformStateForV3.
        // Outbound validation is enabled on AGV controllers to verify V3.0 schema compliance.
        // Inbound validation on MC controllers is also enabled.
        ...
        const agvController2 = new AgvController(agvId2, v30Opts(t),
        ...
        const agvController3 = new AgvController(agvId3, v30Opts(t),
        ...
        await t.test("visualization received while not driving", ts => new Promise(async resolve => {
            ...
            await mcController.subscribe(Topic.Visualization, agvId1, async (vis, _agvId, _topic, subscriptionId) => {
                invocations++;
                ts.strictSame((vis as any).mobileRobotPosition, { x: 0, y: 0, theta: 0, mapId: "local", localized: true });
        ...
        await t.test("instant action pick rejected with error", ts => new Promise(async resolve => {
            ...
            let errorInvocations = 0;
            const actions = await mcController.initiateInstantActions(agvId1, {
                actions: [createPickDropNoopAction("pick", BlockingType.None)],
            }, {
                onActionStateChanged: () => {
                    ts.fail("onActionStateChanged should never be called");
                    resolve();
        ...
        await t.test("instant action initPosition twice in series", ts => new Promise(async resolve => {
            ...
            const actions = await mcController.initiateInstantActions(agvId1, {
                ...
                onActionStateChanged: (actionState, withError, action, _agvId, state) => {
                    ...
                    if (actionStateInvocations === 1) {
                        ...
                        ts.strictSame((state as any).mobileRobotPosition, { x: 10, y: 10, theta: 0, mapId: "floor2", localized: true });
                        ...
                        ts.equal(actionState.resultDescription, "Position initialized");
                        ts.strictSame((state as any).mobileRobotPosition, { x: 0, y: 0, theta: 0, mapId: "local", localized: true });
        ...
        // In VDA 5050 v3.0, instant actions can only have blockingType NONE, so
        // we send startPause and stopPause as separate calls to ensure sequential execution.
        await t.test("instant actions startPause-stopPause in series", ts => new Promise(async resolve => {
            const startPauseActions = await mcController.initiateInstantActions(agvId1, {
                actions: [{
                    actionId: createUuid(),
                ...
                onActionStateChanged: async (actionState, withError, action, _agvId, state) => {
                    ts.strictSame(action, startPauseActions.actions[0]);
                    ts.equal(withError, undefined);
                    ts.equal(actionState.actionId, startPauseActions.actions[0].actionId);
                    ts.equal(actionState.actionStatus, ActionStatus.Finished);
                    ts.equal(actionState.resultDescription, "Paused");
                    ts.equal(state.paused, true);

                    const stopPauseActions = await mcController.initiateInstantActions(agvId1, {
                        actions: [{
                            actionId: createUuid(),
                        ...
                        onActionStateChanged: (actionState1, withError1, action1, _agvId1, state1) => {
                            ts.strictSame(action1, stopPauseActions.actions[0]);
                            ts.equal(withError1, undefined);
                            ts.equal(actionState1.actionId, stopPauseActions.actions[0].actionId);
                            ts.equal(actionState1.actionStatus, ActionStatus.Finished);
                            ts.equal(actionState1.resultDescription, "Unpaused");
                            ts.equal(state1.paused, false);
                            resolve();
                        },
                        onActionError: () => {
                            ts.fail("onActionError should never be called");
        ...
        await t.test("instant actions startCharging-stopCharging", ts => new Promise(async resolve => {
            ...
            const actions = await mcController.initiateInstantActions(agvId1, {
                ...
                onActionStateChanged: async (actionState, withError, action, _agvId, state) => {
                    ...
                    ts.equal((state as any).powerSupply.charging, actionStateInvocations === 1 ? false : true);

                    if (actionStateInvocations === 1) {
                        return;
                    }

                    const rangeOnCharging = (state as any).powerSupply.range;
                    ...
                    await mcController.subscribe(Topic.State, agvId1, async (state1, _agvId1, _topic1, subscriptionId) => {
                        ...
                        if (!isStoppingCharging && (state1 as any).powerSupply.stateOfCharge > 90) {
                            ...
                            const actions1 = await mcController.initiateInstantActions(agvId1, {
                                ...
                                onActionStateChanged: async (actionState1, withError1, action1, _agvId11, state11) => {
                                    ...
                                    ts.equal((state11 as any).powerSupply.charging, actionStateInvocations1 === 1 ? true : false);

                                    if (actionStateInvocations1 === 2) {
                                        ts.ok(stateChangesReceived >= 9);
                                        ts.ok((state11 as any).powerSupply.range - rangeOnCharging >= 25920 - 23040);
                                        ts.ok((state11 as any).powerSupply.range - rangeOnCharging <= 28800 - 23040);
                                        await mcController.unsubscribe(subscriptionId);
                                        resolve();
                                    }
                                },
        ...
        // Note: factsheetRequest is not supported by AgvController for v3.0 (only v2.0, v2.1).
        // The controller responds with FAILED status and an error.
        await t.test("instant action factsheetRequest not supported (VDA: AGV V3.0| MC V3.0)", ts => new Promise(async resolve => {
            let errorInvocations = 0;
            const actions = await mcController.initiateInstantActions(agvId1, {
                actions: [{
                    actionId: createUuid(),
                ...
                onActionStateChanged: (actionState, withError, action) => {
                    ...
                    ts.ok(withError.errorDescription.includes("not supported"));
                    resolve();
                },
                onActionError: () => {
                    ts.fail("onActionError should never be called");
        ...
        await t.test("instant action stateRequest (VDA: AGV V3.0| MC V3.0)", ts => new Promise(async resolve => {
            let actionStateInvocations = 0;
            const actions = await mcController.initiateInstantActions(agvId1, {
                actions: [{
                    actionId: createUuid(),
        ...
        await testOrderError(t, "order not executable while charging",
            ...
            undefined,
            { referenceKey: "powerSupply.charging", referenceValue: "true" },
            { referenceKey: "orderUpdateId", referenceValue: "0" },
        );
        ...
        await testOrderError(t, "order not executable as emergency stop is active",
            ...
            undefined,
            { referenceKey: "safetyState.activeEmergencyStop", referenceValue: "MANUAL" },
            { referenceKey: "orderUpdateId", referenceValue: "0" },
        );
